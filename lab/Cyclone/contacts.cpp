...
Vector3 Contact::calculateLocalVelocity(unsigned bodyIndex, real duration)
{
    ...
    // Turn the velocity into contact-coordinates.  (step 1)
    ...
    // Calculate the velocity in contact-coordinates. (Among the linear velocity, extact the linear velocity along the contact coordinate
    accVelocity = contactToWorld.transformTranspose(accVelocity);

    // We ignore any component of acceleration in the contact normal
    // direction, we are only interested in planar acceleration
    accVelocity.x = 0;  // (step 2)
...
void Contact::calculateInternals(real duration)
{
    ...
    // Calculate an set of axis at the contact point.  : build abc matrix (local three axes)
...
void Contact::calculateDesiredDeltaVelocity(real duration)
{
    ...
    // Calculate the acceleration induced velocity accumulated this frame (Linear velocity)
    real velocityFromAcc = 0;  //note that this is scalar

    if (body[0]->getAwake())
    {
        //Project the velocity onto the contact normal and get the length of projected vector (This in only for linear velocity)
