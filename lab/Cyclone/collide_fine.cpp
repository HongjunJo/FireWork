    ...
    )
{
    ...
    //박스의 현재 orientatio을 나타내는 3개의 axis에 대해 axis를 투사(dot) 해서 거리값 구함
    float a1 = axis.dot(box.getAxis(0));
    float a2 = axis.dot(box.getAxis(1));
    float a3 = axis.dot(box.getAxis(2));

    //현재 박스의 크기를 나타내는 벡터를 투사     
    float check = half.dot(Vector3(a1, a2, a3));
    //
    return
